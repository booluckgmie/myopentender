...
const BASE_URL = 'https://www.prasarana.com.my/procurement/open-tenders';
...
async function* scrape() {
  ...
  try {
    ...
    $('table tr, .tender-item, article').each((_, el) => {
      ...
      let title, link, deadline, openDate, ref;
      if (tds.length) {
        ...
        link = $(el).find('a[href]').attr('href');
        const dateText = $(el).find('.closing, .deadline, time').first().text().trim();
        deadline = parseDate(dateText);
      }
      if (!title || title.length < 15) return;
      let url = link || BASE_URL;
      if (url.startsWith('/')) url = 'https://www.prasarana.com.my' + url;
      results.push({ source_id: SOURCE_ID, ref: ref || null, title,
